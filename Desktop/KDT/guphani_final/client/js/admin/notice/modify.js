...
var id = herf.split('=')[1]
let link = `https://www.guphani.com/admin/notice/modify?id=${id}`;
...
const editor = new toastui.Editor({
    ...
    hooks: {
        addImageBlobHook: async (blob, callback) => {
            ...
            try {
                const response = await fetch('https://www.guphani.com/admin/notice/modify/imgUpload', {
                    method: 'POST',
                    body: formData,
                });
...
async function handleEditor(blob, callback) {
    ...
    const data = await fetch(`https://www.guphani.com/admin/notice/modify/${id}`, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json",
            'Authorization': `Bearer ${token}`,
