...
idCheck.addEventListener('click',()=>{
    ...
    // const response = await fetch('https://port-0-guphani-final-1gksli2alpullmg3.sel4.cloudtype.app:8080/auth/findById', {
    //     method: 'POST',
    //     headers: {
...
hpCheck.addEventListener('click', async function(event) {
    ...
    if(hp.length < 10 || hp.length > 11){
        ...
        try {
            // phnumber로 인증번호 전송
            const response = await fetch('https://port-0-guphani-final-1gksli2alpullmg3.sel4.cloudtype.app:8080/auth/user/sendVerification', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',  
                },
...
hpCheckNumber.addEventListener('click',async function(event) {
    ...
    try{
        const response = await fetch('https://port-0-guphani-final-1gksli2alpullmg3.sel4.cloudtype.app:8080/auth/user/verifyCode', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    if (signUpForm) {
        signUpForm.addEventListener('click', async () => {
            ...
            try {
                ...
                const response = await fetch('https://port-0-guphani-final-1gksli2alpullmg3.sel4.cloudtype.app:8080/auth/user/signUp', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
