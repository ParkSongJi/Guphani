...
function setKakaoMapMain(idName, lat, lng) {
  try {
    ...
    console.error('지도 설정 중 오류 발생:', error.message);
  }
}
...
function getCurrentLocation() {
  try {
    ...
    console.error('현재 위치 확인 중 오류 발생:', error.message);
  }
}
getCurrentLocation();
...
detailBack.addEventListener('click', () => {
  try {
    ...
    console.error('디테일 뒤로가기 오류:', error.message);
  }
});
...
socket.on('updateData', (newData) => {
  try {
    ...
    console.error('데이터 업데이트 오류:', error.message);
  }
});
...
function updateErList(data) {
  try {
    ...
    console.error('응급실 목록 업데이트 오류:', error.message);
  }
}
...
`
    ...
    console.error('목록 업데이트 오류:', error.message);
  }
  //createInfoFunc 끝
}
...
function setKakaoMapDetail(idName, lat, lng) {
  try {
    ...
    console.error('지도 설정 오류:', error.message);
  }
}
...
function makeHomeMarker(map, lat, lng) {
  try {
    ...
    console.error('홈 마커 생성 오류:', error.message);
  }
}
...
locationBtn.addEventListener('click', () => {
  try {
    ...
    console.error('현재 위치 가져오기 오류:', error.message);
  }
});
